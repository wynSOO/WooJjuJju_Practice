package app.com.godata_view.service;
...
/*
    ...
    클래스의 method를 직접 사용하는 것은 좋지 않다.
    안드로이드에 부담이 되고, 전체적인 App의 성능을 떨어 뜨린다.

    만약 직접 method를 호출할 경우
    app은 정상적으로 실행되지 않고 중단 되어 버린다.

    그래서
    AsyncTask 라는 클래스의 도움을 받아서 실행해야한다.
...
public class GoDataGetterService extends AsyncTask{
    ...
    public void getData(){
        ...
        // Java String 변수를 생성하고 계속 이어붙이기를 하는 것은 성능상 문제가있다.
        /*
            String name = " 홍길동 ";
            name += " 이몽룡 ";
            name += " 성춘향 " ;
